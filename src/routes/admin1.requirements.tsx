...
export const Route = createFileRoute("/admin1/requirements")({
  component: Admin1RequirementsPage,
});

function Admin1RequirementsPage() {
  const docs = useEventDocs().filter((doc) => doc.reviewStatus === "pending_admin1");

  return (
    <>
      <PageHead title="Requirements final approval" sub="Approve compliance-cleared requirement trackers as final authority." />
      <Panel>
        {docs.length === 0 ? (
          <EmptyState title="No requirements trackers pending Admin 1 review" sub="Admin 2-cleared requirement packets will appear here." icon={ClipboardCheck} />
        ) : (
            ...
            {docs.map((doc) => (
              <div key={doc.id} className="rounded-2xl border border-border bg-card p-4">
                ...
                <AppButton asChild variant="secondary" size="sm" className="mt-3">
                  <Link to="/admin1/requirements/$eventId" params={{ eventId: doc.id }}>
