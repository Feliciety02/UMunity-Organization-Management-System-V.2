...
export const Route = createFileRoute("/adviser/transitions/$transitionId")({
  component: AdviserTransitionDetailPage,
});

function AdviserTransitionDetailPage() {
  ...
  if (!workflow || !session) {
    return (
      <>
        ...
        <Panel>
          <p className="text-sm text-muted-foreground">Return to the adviser transition queue and try again.</p>
        </Panel>
      </>
    );
  }
  ...
  return (
    <OfficerTransitionDetail
      ...
      backTo="/adviser/transitions"
      backLabel="Transition queue"
    />
  );
}
